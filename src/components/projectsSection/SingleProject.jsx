...
const SingleProject = ({ name, title , subtitle, align, image, link }) => {
  return (
    <motion.div
    ...
    >
      <div>
        ...
        >
          <h2 className="md:text-3xl md:font-bold font-bold sm:text-2xl text-orange ">{name}</h2>
          <p className="md:text-xl">{title}</p>
          <p className="md:text-sm">{subtitle}</p>
        </h2>
        <a
          href={link}
        ...
        >
          Tap to Know More! <BsFillArrowUpRightCircleFill />
        </a>
      </div>
      <div className="max-h-[220px] max-w-[400px] rounded-xl overflow-hidden hover:scale-110 transform transition-all duration-500 ralative border border-white">
        ...
        <img src={image} alt="website image" className="w-[300px] h-[200px] md:w-[400px] md:h-[100px]" />
      </div>
    </motion.div>
  );
};
